...
def plot_losses(epochs_seen, tokens_seen, train_losses, val_losses):
    ...
    plot_path = RESULT_PREFIX + "loss-plot-standalone.pdf"
...
def main(test_mode=False):
    ...
    # 它用来指定如何将一个批次（batch）中的多个样本（样本通常是一个数据集中的单个元素）合并成一个批量。默认情况下，DataLoader 会将样本沿着第一个维度（通常是样本维度）堆叠在一起。然而，在许多情况下可以自定义 collate_fn。
    ...
    test_data_path = RESULT_PREFIX + "instruction-data-with-response-standalone.json"
    with open(test_data_path, "w") as file:
        json.dump(test_data, file, indent=4)  # "indent" for pretty-printing
    print(f"Responses saved as {test_data_path}")

    model_path = RESULT_PREFIX + f"{re.sub(r'[ ()]', '', CHOOSE_MODEL) }-sft-standalone.pth"
